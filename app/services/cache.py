...
from app.services.circuit_breaker import CircuitBreaker, redis_circuit_breaker
...
class CacheManager:
    ...
    def __init__(
        ...
        self.circuit_breaker = circuit_breaker or redis_circuit_breaker
        self._client: Optional[aioredis.Redis] = None
        self._is_fake = False

    async def get_client(self) -> aioredis.Redis:
        """Lazily initializes and returns an async Redis client, with fakeredis fallback ONLY in development/test."""
        if self._client is None:
            try:
                ...
                if settings.is_development_or_test:
                    logger.warning(
                        f"Could not connect to Redis at {self.redis_url}: {e}. "
                        f"APP_ENV='{settings.APP_ENV}' permits in-memory FakeRedis fallback for local dev/testing."
                    )
                    ...
                    logger.error(
                        f"CRITICAL: Failed to connect to Redis at {self.redis_url} in '{settings.APP_ENV}' environment: {e}. "
                        "Refusing silent FakeRedis fallback in production."
                    )
                    raise e
        return self._client
    ...
    async def get_url(self, short_code: str) -> Optional[Dict[str, Any]]:
        ...
        if not self.circuit_breaker.can_execute():
            logger.info(f"Circuit breaker [{self.circuit_breaker.name}] OPEN. Skipping Redis cache lookup.")
            return None
